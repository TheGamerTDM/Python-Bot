...
class Settings(commands.Cog):
    ...
    async def prefix(self, ctx, prefix: str):
        """Sets the server-wide prefix. 60 sec cooldown."""
        ...
        embed = discord.Embed(title="Setting updated", description=f'Server-wide prefix is now {prefix}',
                              color=self.bot.config.color, timestamp=datetime.utcnow())
    ...
    @commands.is_owner()
    @commands.command(aliases=['block', 'unenableuserfrompermission'])
